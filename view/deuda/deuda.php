...
<table class="table table-striped table-bordered display responsive nowrap datatable">
    ...
    <tbody>
    <?php foreach($this->model->ListarDeudaAgrupado() as $r): 
    // echo '<pre>'; var_dump($r); echo '</pre>';
        if($r->moneda == 'USD'){
            $moneda = ' USD';
        }else if($r->moneda == NULL || $r->moneda == 'GS'){
            $moneda = ' GS';
        }else{
            $moneda = ' RS';
        }
        ...
        <tr class="click">
            ...
            <td><?php echo number_format($r->montos,0,".",","); echo $moneda; ?></td>
            <td><?php echo number_format($r->saldos,0,".",","); echo $moneda; ?></td>
            <td><?php echo date("d/m/Y H:i", strtotime($r->fecha)); ?></td>
            <td><?php echo (date("Y", strtotime($r->vencimiento))>2000)? date("d/m/Y", strtotime($r->vencimiento)):""; ?></td>
            <?php if ($r->id_venta): ?>
            <td>
                <a href="#cobrosModal" class="btn btn-success" data-toggle="modal" data-target="#cobrosModal" data-id="<?php echo $r->id; ?>" data-cliente="<?php echo $r->id_cliente; ?>">Cobros</a>
                <a href="#detallesModal" class="btn btn-default" data-toggle="modal" data-target="#detallesModal" data-id="<?php echo $r->id_venta; ?>">Última venta</a>
            </td>
            <?php else: ?>
            <td>
                <a href="#cobrosModal" class="btn btn-success" data-toggle="modal" data-target="#cobrosModal" data-id="<?php echo $r->id; ?>" data-cliente="<?php echo $r->id_cliente; ?>">Cobros</a>
