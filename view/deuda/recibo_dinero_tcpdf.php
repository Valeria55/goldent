...
// Evitar que warnings/espacios rompan la salida del PDF
if (ob_get_level() === 0) {
    ob_start();
}

class ReciboDineroTCPDF extends TCPDF
{
...
try {
    ...
    // Usamos ingresos solo para obtener el ID del ingreso (comprobante 000 + ingreso)
    $detalle_dinero = $deuda_model->obtenerDetalleReciboDinero($grupo_pago_id, $permitir_anulado);
    $ingresos = $detalle_dinero['ingresos'] ?? [];

    // Y usamos el detalle de deudas/ventas para listar FACTURAS en la tabla (Factura/Fecha/Importe)
    $detalle_recibo = $deuda_model->obtenerDetalleRecibo($grupo_pago_id, $permitir_anulado);
    $detalle_deudas = $detalle_recibo['detalle_deudas'] ?? [];

    if (empty($ingresos)) {
        die('No se encontraron ingresos para este comprobante');
    }

    if (empty($detalle_deudas)) {
        die('No se encontraron facturas/deudas para este comprobante');
    }

    $primer_ingreso = $ingresos[0];
    $numero_comprobante = isset($primer_ingreso->id_ingreso) ? ('000' . $primer_ingreso->id_ingreso) : 'SIN-NUMERO';

    $primera_deuda = $detalle_deudas[0];

    $pdf = new ReciboDineroTCPDF();
    $pdf->AddPage();

    function generarReciboDinero($pdf, $detalle_deudas, $numero_comprobante, $primera_deuda, $y_offset, $tipo, $es_anulado = false)
    {
        $total_general = 0;
        ...
        // TABLA DERECHA (FACTURAS)
        $tabla_x = 110;
        $pdf->SetFont('helvetica', 'B', 7);
        $pdf->Rect($tabla_x, $y_start, 37, 5, 'D'); // FACTURA N°
        $pdf->Rect($tabla_x + 37, $y_start, 27, 5, 'D'); // FECHA
        $pdf->Rect($tabla_x + 64, $y_start, 26, 5, 'D'); // IMPORTE
        ...
        foreach ($detalle_deudas as $deuda) {
            $pdf->Rect($tabla_x, $y_pos, 37, 5, 'D');
            $pdf->Rect($tabla_x + 37, $y_pos, 27, 5, 'D');
            $pdf->Rect($tabla_x + 64, $y_pos, 26, 5, 'D');

            $pdf->SetXY($tabla_x + 1, $y_pos + 1);
            $pdf->Cell(33, 3, $deuda->nro_comprobante ?: 'N/A', 0, 0, 'L');

            $pdf->SetXY($tabla_x + 36, $y_pos + 1);
            $fecha_formateada = !empty($deuda->deuda_fecha) ? date('d/m/Y', strtotime($deuda->deuda_fecha)) : '';
            $pdf->Cell(23, 3, $fecha_formateada, 0, 0, 'C');

            $pdf->SetXY($tabla_x + 68, $y_pos + 1);
            $monto_aplicado = (float)($deuda->monto_aplicado ?? 0);
            $pdf->Cell(18, 3, number_format($monto_aplicado, 0, '.', '.'), 0, 0, 'R');

            $total_general += $monto_aplicado;
            $y_pos += 5;
        }

        $filas_usadas = count($detalle_deudas);
        ...
        $pdf->Cell(36, 3, $primera_deuda->id_cliente ?: '', 0, 1, 'L');
        ...
        $pdf->Cell(110, 4, strtoupper((string)($primera_deuda->cliente_nombre ?? '')), 0, 1, 'L');

        $pdf->SetXY(10, $y_left + 10);
        $pdf->Cell(110, 4, 'R.U.C. / C.I.N°: ' . ($primera_deuda->cliente_documento ?: ''), 0, 1, 'L');

        $pdf->SetXY(10, $y_left + 15);
        $pdf->Cell(110, 4, 'La Cantidad de Guaranies:', 0, 1, 'L');

        $pdf->SetFont('helvetica', 'B', 7);
        $total_en_texto = numeroALetras((int)round($total_general));
        $pdf->SetXY(10, $y_left + 20);
        $pdf->Cell(110, 4, strtoupper($total_en_texto) . ' * * * * * * * * * * * * * *', 0, 1, 'L');

        $pdf->SetFont('helvetica', '', 7);
        $pdf->SetXY(10, $y_left + 26);
        $pdf->Cell(110, 4, 'En concepto de cobro según detalle.', 0, 1, 'L');
    ...
    generarReciboDinero($pdf, $detalle_deudas, $numero_comprobante, $primera_deuda, 0, 'ORIGINAL', !empty($anulado));
    $pdf->Line(10, 140, 200, 140);
    generarReciboDinero($pdf, $detalle_deudas, $numero_comprobante, $primera_deuda, 140, 'DUPLICADO', !empty($anulado));

    $filename = 'Recibo_Dinero_' . $grupo_pago_id;
    if (!empty($anulado)) {
        $filename .= '_ANULADO';
    }
    $filename .= '.pdf';
    ...
    die('Error al generar el recibo de dinero: ' . $e->getMessage());
}
