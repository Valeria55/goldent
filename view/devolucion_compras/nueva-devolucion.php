...
?>
<h1 class="page-header"> Nueva devolución - compra <?php echo $id_compra ?></h1>
<div class="container">
    <div class="row" >
        <form method="post">
            <input type="hidden" id="id_compra" name="id_compra" value="<?php echo $id_compra ?>">
            <input type="hidden" name="c" value="devolucion_tmpcompras">
            ...
            <select name="id_producto" id="producto" class="form-control selectpicker" data-show-subtext="true" data-live-search="true" data-style="form-control"
                ...
                <?php foreach($this->producto->ListarcompraProducto($id_compra) as $producto): ?> 
                <option style="font-size: 18px" data-subtext="<?php echo $producto->id; ?>" value="<?php echo $producto->id; ?>"><?php echo $producto->producto.' ( '.$producto->cantidad.' ) - '.number_format($producto->precio_minorista,0,".","."); ?> </option>
                <?php endforeach; ?>
            </select>
        </div>
            <div class="col-sm-3">
             ...
             <div class="col-sm-3">
                <label>Precio</label>
               <input type="number" name="precio_compra" class="form-control" id="precio_compra" value="">   
            </div>
    ...
    <?php
     ...
     foreach($this->model->Listar() as $r): 
        ...
        <tr>
            ...
            <td>
                <a  class="btn btn-danger" onclick="javascript:return confirm('¿Seguro de eliminar este registro?');" href="?c=devolucion_tmpcompras&a=Eliminar&id=<?php echo $r->id; ?>&id_compra=<?php echo $id_compra; ?>">Cancelar</a>
            </td>
        </tr>
        <input type="hidden" id="clienteId" value="<?php echo $r->id_compra; ?>">
    <?php endforeach; ?>
...
<?php if($subtotal>0){ ?>
<div align="center"><a class="btn btn-lg btn-primary " href="#devolucionModal" class="btn btn-success" data-toggle="modal" data-target="#devolucionModal" data-c="devolucion_compras">Finalizar (F4)</a></div>
<?php } ?>
</div> 
</div>
</div>

<?php include("view/devolucion_compras/finalizar-modal-devolucion.php"); ?>
...
<script type="text/javascript">
    ...
    $('#producto').on('change',function(){
        ...
        //alert(id_producto);
        var url = "?c=compra&a=ObtenerProducto&id_compra="+id_compra+"&id_producto="+id_producto;
        console.log(url);
            $.ajax({
                ...
                success:function(respuesta){
                    ...
                    $("#precio_compra").html(producto.precio_compra);
