...
<h1 class="page-header">Nueva transferencia
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        // Evento change del select con id "producto"
        // Este evento sirve para agregar en el input hidden el valor seleccionado del select correspondiente a codigo.
        $('#producto').on('change', function() {
            var codigoSeleccionado = $(this).find(':selected').data('subtext');
            $('#codigo_producto').val(codigoSeleccionado);
        });
    });
    ...
    $('.cancelar').on('click', function() {
     ...
     $('#productoNuevo').submit(function(e) {
        ...
        if (productoYaAgregado(codigo)) {
            // Product already added, show SweetAlert
            Swal.fire({
                icon: 'warning',
    ...
    function productoYaAgregado(codigo) {
        var productos = $('#tabla_items').find('td:first-child').map(function() {
            return $(this).text().trim();
        }).get();
        return productos.includes(codigo);
    }
    function calcular() {
