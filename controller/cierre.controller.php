...
class cierreController{
    ...
    public function Obtener(){
        ...
        if(isset($_REQUEST['id'])){
            $cierre = $this->model->Obtener($_REQUEST['id']);
        }
        
        require_once 'view/cierre/cierre-editar.php';
     ...
     public function EditarCierre(){
        ...
        $cierre->id = $_REQUEST['id'];
        $cierre->monto_apertura = $_REQUEST['monto_apertura'];
        $cierre->apertura_rs = $_REQUEST['apertura_rs'];
        $cierre->apertura_usd = $_REQUEST['apertura_usd'];
        
        $cierre->monto_cierre = $_REQUEST['monto_cierre'];
        $cierre->monto_dolares = $_REQUEST['monto_dolares'];
        $cierre->monto_reales = $_REQUEST['monto_reales'];
        
        $this->model->Actualizar($cierre);
    ...
    public function ActualizarCotizacion(){
        ...
        session_start();
        $cierre->cot_dolar_tmp = $_REQUEST['cot_dolar_tmp']??null;
        $cierre->cot_real_tmp = $_REQUEST['cot_real_tmp']??null;

        if(!is_null( $cierre->cot_dolar_tmp)){
            $this->model->ActualizarDolar($cierre->cot_dolar_tmp, $this->model->UltimoUsuario($_SESSION['user_id'])->id);
        }
        if(!is_null( $cierre->cot_real_tmp)){
            $this->model->ActualizarReal($cierre->cot_real_tmp, $this->model->UltimoUsuario($_SESSION['user_id'])->id);
        }
