...
class venta_tmp
{
	...
	public function Listar()
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT v.prod_factura, v.can_factura, v.id_presupuesto, v.id, v.id_producto, c.codigo, v.id_vendedor, v.descuento, c.precio_costo, v.precio_venta, c.producto, c.precio_costo, v.cantidad, v.id_venta, v.id_sucursal FROM ventas_tmp v LEFT JOIN productos c ON v.id_producto = c.id WHERE id_vendedor = ? ORDER BY v.id DESC");
	...
	public function ObtenerMonto()
	{
		try 
		{
			...
			$stm = $this->pdo
			          ->prepare("SELECT *, SUM((precio_venta*cantidad)-(descuento*cantidad)) AS monto FROM ventas_tmp  WHERE id_vendedor ='$user_id' GROUP BY id_venta");
