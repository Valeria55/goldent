...
class compra
{
	...
	public function ObtenerDevolucion($id_compra)
	{
		try {
			$stm = $this->pdo->prepare("SELECT COUNT(c.id_compra) > 0 AS existe_registro
			FROM compras c 
			LEFT JOIN devoluciones_compras d ON c.id_compra = d.id 
			WHERE d.id_compra = ?");
	...
	public function ObtenerProducto($id_compra, $id_producto)
	{
		try {
			$stm = $this->pdo
				->prepare("SELECT * FROM compras WHERE id_compra = ? AND id_producto = ?");
	...
	public function AgrupadoProducto($desde, $hasta)
	{
		try {
			...
			$stm = $this->pdo->prepare("SELECT v.fecha_compra, v.contado, p.producto, SUM(v.cantidad) as cantidad, v.precio_compra, SUM(v.total) as total FROM compras v LEFT JOIN productos p ON v.id_producto = p.id LEFT JOIN clientes c ON v.id_cliente = c.id WHERE CAST(v.fecha_compra AS date) >= ? AND CAST(v.fecha_compra AS date) <= ? AND v.anulado = 0 GROUP BY v.id_producto ORDER BY v.id_compra DESC");
	...
	public function Registrar($data)
	{
		try {
			...
			$sql = "INSERT INTO compras (id_compra, id_cliente, id_vendedor, id_producto, precio_compra, precio_min, precio_may, subtotal, descuento, iva, total, comprobante, nro_comprobante, cantidad, margen_ganancia, fecha_compra, metodo, banco, contado, cot_usd, cot_rs) 
		        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
